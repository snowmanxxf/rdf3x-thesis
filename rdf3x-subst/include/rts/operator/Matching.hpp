#ifndef H_rts_operator_Matching
#define H_rts_operator_Matching
#define KEYSIZE 8
//---------------------------------------------------------------------------
// RDF-3X
...
class Matching : public Operator
{
   private:
   /// A hash table entry
   struct Entry {
   ...
   class BuildHashTable : public Scheduler::AsyncPoint {
      ...
      public:
      /// Constructor
      BuildHashTable(Matching& join) : join(join),done(false) {}
      /// Perform the task
      void run();
   };
   ...
   class ProbePeek : public Scheduler::AsyncPoint {
      ...
      public:
      /// Constructor
      ProbePeek(Matching& join) : join(join),count(0),done(false) {}
      /// Perform the task
      void run();
   };
   ...
   /// Assigns true or false to weather body tuple matches with head
   Register* match;
   ...
   public:
   /// Constructor
   Matching(Register* match,Operator* left,std::vector<Register*> leftJoinKeys,const std::vector<Register*>& leftTail,Operator* right,std::vector<Register*> rightJoinKeys,double hashPriority,double probePriority,double expectedOutputCardinality);
   /// Destructor
   ~Matching();
